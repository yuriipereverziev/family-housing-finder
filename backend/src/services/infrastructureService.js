...
function isPointInPolygon(lat, lon, polygon) {
    if (!polygon || polygon.length === 0) {
        return true; // Якщо немає полігону - вважаємо що всередині
    }
...
// Отримати дані з кешу або Overpass API
export async function getInfrastructureData(city, districtName, facilityType, district) {
    // 1. Спробувати знайти в MongoDB кеші
    const cached = await InfrastructureCache.findOne({
        city,
        districtName,
        facilityType,
    ...
    // 2. Отримуємо полігон району з БД
    const districtFromDB = await District.findOne({
        city,
        name: districtName
    }).select('polygon').lean();

    const polygon = districtFromDB?.polygon?.[0] || null;

    // 3. Якщо немає в кеші - завантажити з Overpass API
    console.log(`⏳ Завантаження з Overpass API: ${facilityType} для ${districtName}`);

    try {
        const markers = await fetchFromOverpass(facilityType, district, polygon);

        // 4. Зберегти в MongoDB
        await InfrastructureCache.findOneAndUpdate(
            { city, districtName, facilityType },
            {
                city,
...
// Завантажити дані з Overpass API
async function fetchFromOverpass(facilityType, district, polygon) {
    const config = FACILITY_CONFIG[facilityType];
    if (!config) {
        throw new Error(`Невідомий тип: ${facilityType}`);
    }

    // ✅ ЗМЕНШЕНО радіус до 2000м для більшої точності
    const query = `
        [out:json][timeout:40];
        (
            node["${config.key}"="${config.value}"](around:2000,${district.lat},${district.lon});
            way["${config.key}"="${config.value}"](around:2000,${district.lat},${district.lon});
            relation["${config.key}"="${config.value}"](around:2000,${district.lat},${district.lon});
        );
        out center;
    `;
    ...
    for (const apiUrl of OVERPASS_APIS) {
        try {
            ...
            let markers = response.data.elements
                .filter(el => el.lat || el.center?.lat)
                .map(el => ({
                    lat: el.lat || el.center.lat,
                    lon: el.lon || el.center.lon
            ...
            // ✅ ФІЛЬТРУЄМО за полігоном
            if (polygon && polygon.length > 0) {
                const beforeCount = markers.length;
                markers = markers.filter(m => isPointInPolygon(m.lat, m.lon, polygon));
                console.log(`   🔍 Відфільтровано: ${beforeCount} → ${markers.length} (по полігону)`);
            }
